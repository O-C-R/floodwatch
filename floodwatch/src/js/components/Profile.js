...
export class ProfileExplanation extends Component {
  ...
  render() {
    return (
      <div>
        <p>
          Donate your data to help us discover discriminatory patterns in advertising, and reverse the power relationship between people and advertisers.
        </p>
        <p>
          Wondering why your demographic data matters?
          {' '}
          <a onClick={this.toggleDescriptionVisibility.bind(this)}>
            Learn more
          </a>
        </p>
...
export class DemographicContainer extends Component {
  ...
  componentDidMount() {
    const init = async () => {
      try {
        ...
        const filteredUserData = _.pick(userData, TO_PICK);
  ...
  async updateUserInfo() {
    try {
      if (this.state.userData) {
        ...
        const reply = await FWApiClient.get().updatePersonDemographics(
          userData,
        );

        const filteredUserData = _.pick(reply, TO_PICK);
        this.setState({ userData: filteredUserData });
        ...
        this.statusHandler(
          'error',
          'Profile not loaded, please reload this page.',
        );
      }
    } catch (e) {
      this.statusHandler(
  ...
  render() {
    const elems = Filters.filters.map((filter, i) => {
      if (filter.name === 'age') {
        return (
          <AgeOption
...
export class AccountOptionsContainer extends Component {
  render() {
    return (
      <div className="panel-container">
        <p>
          If you would like to download your data, reset your password, or delete your account, please email us at
          {' '}
