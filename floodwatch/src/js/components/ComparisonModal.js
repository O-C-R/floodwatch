...
export class ComparisonModal extends Component {
  ...
  render() {
    const sentence = `${createSentence(this.props.currentSelectionLeft)} compared to ${createSentence(this.props.currentSelectionRight)}.`;
    return (
      <Modal show={this.props.visible} className="static-modal" bsSize="lg">
        <Modal.Header>
          <Modal.Title>Change Comparison</Modal.Title>
          <p>
            Select new demographic groups to compare. Try one of our presets, or select Custom to make your own.
          </p>
        </Modal.Header>
          ...
          <Row>
            ...
            <Col xs={5}>
              <ModalSegment
                ...
                filterData={Filters}
                currentSentence={createSentence(
                  this.props.currentSelectionRight,
                )}
                handlePresetClick={this.changeCategoriesPreset.bind(this)}
        ...
        <Modal.Footer>
          ...
          <Button bsStyle="primary" onClick={this.props.toggleModal}>
            Done
          </Button>
        </Modal.Footer>
      </Modal>
